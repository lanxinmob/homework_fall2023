...
class SoftActorCritic(nn.Module):
    ...
    def q_backup_strategy(self, next_qs: torch.Tensor) -> torch.Tensor:
        ...
        if self.target_critic_backup_type == "doubleq":
            ...
            selected_indices = random_indices[:2]
            selected_qs = next_qs.gather(
                dim=0,
                index=selected_indices,
            )
            next_qs = selected_qs.min(dim=0).values
    ...
    def update_critic(
        ...
        with torch.no_grad():
            ...
            target_values: torch.Tensor = reward.unsqueeze(0) + self.discount*(1-done.float())*next_qs
