    ...
    def _calculate_q_vals(self, rewards: Sequence[np.ndarray]) -> Sequence[np.ndarray]:
        ...
        for rewards in rewards:
            if self.use_reward_to_go:
                path_q_values = self._discounted_reward_to_go(rewards)
            else:
                path_q_values = self._discounted_return(rewards)

            q_values.append(path_q_values)
        ...
        if self.critic is None:
            ...
            if self.gae_lambda is None:
                ...
                for i in reversed(range(batch_size)):
                    ...
                    delta = rewards[i] + self.gamma*values[i+1] - values[i] * (1-terminals[i])
                    advantages[i] = delta + advantages[i+1]*self.gamma*self.gae_lambda* (1-terminals[i]) 
